...
NKP_DRUG_DOWNLOAD_URL = "https://nkp.rsl.ru/api/v1/publication-drugs"
NKP_FOREIGN_AGENTS_DOWNLOAD_URL = "https://nkp.rsl.ru/api/v1/publication-foreign-agent"
...
# Имя CACHE_FILENAME используется внешними сценариями загрузки реестра веществ.
CACHE_FILENAME = DRUG_CACHE_FILENAME

_USER_AGENT = (
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
    "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36 "
    f"IRBIS64-Control/{__version__}"
)
...
def _request(url: str, *, timeout: int = 25) -> tuple[bytes, str, str]:
    ...
    with urllib.request.urlopen(request, timeout=timeout, context=system_tls_context()) as response:
...
def _extract_urls(text: str, base_url: str, *, keywords: Iterable[str] = ()) -> list[str]:
    ...
    # Ищем ссылки как в HTML-атрибутах, так и внутри JS/JSON.
...
def _json_registry_rows(payload: bytes) -> list[list[str]] | None:
    ...
    normalized_aliases = {
        header: {re.sub(r"[^a-zа-я0-9]+", "", alias.casefold()) for alias in names} for header, names in aliases.items()
    }
    ...
    for items in _iter_json_lists(value):
        if len(items) < 2:
            continue
        keys = {str(key) for item in items[:20] for key in item}
        ...
        if "Заглавие" in mapping and ("Автор" in mapping or "ISBN" in mapping) and (best is None or score > best[0]):
...
def _diagnostic_error(stage: str, exc: BaseException, *, url: str = "") -> str:
    """Формирует безопасную однострочную причину для пользовательского журнала."""

    detail = " ".join(str(exc).split()) or "без описания"
    location = f" [{url}]" if url else ""
    return f"{stage}{location}: {type(exc).__name__}: {detail}"
    ...
    for url in urls:
        try:
            ...
            if errors is not None:
                errors.append(_diagnostic_error("запрос выгрузки", exc, url=url))
            continue
        if _looks_like_excel(payload, content_type):
            try:
                ...
                if errors is not None:
                    errors.append(_diagnostic_error("проверка XLSX", exc, url=final_url))
                continue
        ...
        if rows:
            try:
                ...
                if errors is not None:
                    errors.append(_diagnostic_error("преобразование JSON в XLSX", exc, url=final_url))
                continue
            return final_url, count
        if errors is not None:
            content_kind = content_type or "не указан"
            errors.append(
                f"ответ выгрузки [{final_url}] не распознан как XLSX или реестр JSON "
                f"(Content-Type: {content_kind}; байт: {len(payload)})"
            )
    return None
...
def _fetch_nkp_registry(
    ...
    *,
    ...
    # Кнопки «Скачать список» на сайте НКП обращаются к этим API и возвращают
    # полные XLSX. HTML-таблица страницы содержит лишь одну порцию из 50 строк.
    direct_result = _try_candidate_urls(download_urls, target, errors=errors)
    if direct_result:
        source_url, rows = direct_result
    ...
    try:
        ...
        if json_rows:
            try:
                ...
                errors.append(_diagnostic_error("проверка JSON со страницы", exc, url=final_url))
            else:
                _write_meta(meta_path, page_url, final_url, rows)
                return NkpRegistryResult(target, final_url, False, f"получено строк: {rows:,}")
        ...
        if table:
            try:
                ...
                errors.append(_diagnostic_error("проверка HTML-таблицы", exc, url=final_url))
            else:
                _write_meta(meta_path, page_url, final_url, rows)
                return NkpRegistryResult(target, final_url, False, f"получено строк: {rows:,}")
    except Exception as exc:
        errors.append(_diagnostic_error("запрос страницы НКП", exc, url=page_url))
        ...
        try:
            ...
            if table:
                try:
                    ...
                    errors.append(_diagnostic_error("проверка таблицы из браузера", exc, url=page_url))
                else:
                    _write_meta(meta_path, page_url, page_url, rows)
                    return NkpRegistryResult(target, page_url, False, f"получено строк: {rows:,}")
        except Exception as exc:
            errors.append(_diagnostic_error("обработка страницы из браузера", exc, url=page_url))
        ...
        try:
            ...
            if suspicious:
                ...
                try:
                    ...
                    # Метаданные необязательны: сам проверенный кэш остаётся пригодным.
                    source_url = page_url
            ...
            errors.append(_diagnostic_error("проверка сохранённого кэша", exc, url=str(target)))
            target.unlink(missing_ok=True)
            meta_path.unlink(missing_ok=True)
    ...
    if bundled.is_file():
        try:
            ...
            errors.append(_diagnostic_error("проверка встроенной копии", exc, url=str(bundled)))

    diagnostics = "; ".join(errors[-8:]) if errors else "причина не определена"
    raise RuntimeError(
        f"Не удалось получить актуальный {registry_label} с НКП РГБ. "
        f"Проверьте доступ к {page_url} и повторите запуск. Диагностика: {diagnostics}"
    )
...
def fetch_nkp_drug_registry(
    ...
    *,
    ...
    return _fetch_nkp_registry(
        ...
        download_urls=(NKP_DRUG_DOWNLOAD_URL,),
    )
...
def fetch_nkp_foreign_agents_registry(
    ...
    *,
    ...
    return _fetch_nkp_registry(
        ...
        download_urls=(NKP_FOREIGN_AGENTS_DOWNLOAD_URL,),
    )
