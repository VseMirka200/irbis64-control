...
def _record_author_marker(raw_record: str) -> str:
    ...
    for line in re.split(r"\r\n|\n|\r", raw_record):
        ...
        if not match:
            continue
        value = match.group(1)
        if not _has_author_responsibility(value):
            continue
        parts = [
...
def _foreign_result_is_person(result: MatchResult) -> bool:
    ...
    if result.method == "Реестр иностранных агентов: Автор":
        return True
    entry = result.foreign_agent
    return entry is not None and "физичес" in normalize_author(entry.agent_type)
...
def _normalized_marker_text(value: str) -> str:
    # Невидимые Unicode-символы и HTML-сущности не должны создавать дубли меток.
    # Убираем управляющие символы, а разделители приводим к обычному пробелу.
...
def remove_markers_from_tag_values(
    ...
    *,
    ...
    # ^A18+ удаляем только вместе с классификационной меткой приложения.
...
def remove_database_markers(
    ...
    *,
    ...
    # Самостоятельную ^A18+ сохраняем и удаляем её только вместе с нашей меткой.
    conditional_age_removals: dict[int, list[tuple[str, bool]]] = defaultdict(list)
    for field_number in substance_fields | foreign_fields | organization_fields:
        conditional_age_removals[field_number].append(("^A18+", False))
...
def apply_markers_to_tag_values(
    ...
    *,
    ...
    # Визуально одинаковые Unicode-варианты считаем одной меткой.
    ...
    # Авторские метки ^AI^@... сравниваем по смысловому ключу без невидимых символов.
    requested_foreign_keys = {
    ...
    # Удаляем дубли только запрошенных меток, не затрагивая служебные значения.
    for tag, marker in requested:
        matching_indices: list[int] = []
        ...
        # Удаляем только отдельное повторение-дубль, чтобы не потерять соседнее содержимое поля.
        if len(matching_indices) > 1:
            for index in reversed(matching_indices[1:]):
