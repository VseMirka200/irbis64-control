...
from irbis_control.core.review_groups import count_review_record_groups, review_record_group_key
...
def _add_summary_sheet(workbook, summary: ComparisonSummary, *, active: bool = False):
    ...
    rows = [
        ...
        ("Записей для ручной проверки", summary.review_records),
        ("Вариантов совпадений для ручной проверки", summary.review_rows),
    ]
    for row in rows:
...
def _add_review_sheet(
    ...
    *,
    ...
    grouped_results: dict[str, list[MatchResult]] = defaultdict(list)
    for result in review_results:
        grouped_results[review_record_group_key(result)].append(result)

    worksheet.append(REVIEW_MATCH_HEADERS)
    for number, group in enumerate(grouped_results.values(), start=1):
        if number % 500 == 0:
            _cancelled(cancel_cb)
        result = group[0]
        ...
        worksheet.append(
            [
                result.source_type,
                _unique_join(item.method for item in group),
                _unique_join(f"{item.confidence:g}" for item in group),
                _unique_join(item.note or "Недостаточно данных для автоматической установки метки" for item in group),
                _unique_join(item.matched_value for item in group),
                _unique_join(record.authors),
                _unique_join(record.titles),
                record.record_number,
                _unique_join(Path(item.excel.source_file).name for item in group),
                _unique_join(item.excel.sheet_name for item in group),
                _unique_join(str(item.excel.row_number) for item in group),
                _unique_join(item.status for item in group),
            ]
        )

    _style_match_sheet(
        worksheet,
        len(grouped_results) + 1,
        REVIEW_MATCH_HEADERS,
        {
            "A": 24,
            "B": 38,
