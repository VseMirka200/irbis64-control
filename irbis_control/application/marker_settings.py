...
DEFAULT_MARKER_SETTINGS: dict[str, MarkerSetting] = {
    **dict.fromkeys(EXTRA_MATCH_RULES, False),
...
def load_marker_settings(path: str | Path) -> dict[str, MarkerSetting]:
    ...
    for key, default in settings.items():
        value = data.get(key)
        same_scalar_type = (
            isinstance(default, bool) and isinstance(value, bool) or isinstance(default, str) and isinstance(value, str)
        )
        valid_number = (
            isinstance(default, int)
            and type(value) is int
            and (key == "fuzzy_threshold" and 70 <= value <= 100 or key.endswith("_field") and 1 <= value <= 999)
        )
        if same_scalar_type or valid_number:
            settings[key] = value
