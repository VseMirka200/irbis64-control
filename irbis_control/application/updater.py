...
class UpdateError(RuntimeError):
    """Ошибка проверки, загрузки или установки обновления."""
...
def fetch_latest_release(*, timeout: float = 15.0) -> GitHubRelease | None:
...
def schedule_install(downloaded: str | Path, current_executable: str | Path) -> Path:
    """Запускает установку после закрытия собранного приложения."""
    ...
    if package.suffix.casefold() == ".exe" and any(
        ...
        # Inno Setup не может надёжно заменить запущенный EXE. Обёртка
        # дожидается выхода приложения, тихо обновляет его и запускает снова.
        install_commands = f"""
$installer = Start-Process -FilePath {ps_quote(package)} -WorkingDirectory {ps_quote(package.parent)} -ArgumentList @('/VERYSILENT', '/SUPPRESSMSGBOXES', '/NORESTART', '/CLOSEAPPLICATIONS') -Wait -PassThru
if ($installer.ExitCode -ne 0) {{ throw "Установщик завершился с кодом $($installer.ExitCode)." }}
"""
    elif package.suffix.casefold() == ".zip":
