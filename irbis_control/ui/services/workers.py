...
from irbis_control.core.models import ComparisonOptions, ComparisonSummary, MarkerApplicationStats, MatchResult
...
def _marker_neutral_signature(
    ...
    *,
    ...
    """Returns a compact record signature without application-managed markers.

    Some IRBIS servers can briefly return the previous record version from a
    formatted search after markers were removed in the same application
    session.  Comparing marker-neutral fields lets the direct writer accept
    that harmless stale result without overlooking unrelated record edits.
    """
    cleaned, _changed = remove_markers_from_tag_values(
    ...
    for tag, value in cleaned:
        encoded = value.encode("utf-8")
        digest.update(int(tag).to_bytes(4, byteorder="big", signed=False))
        digest.update(len(encoded).to_bytes(8, byteorder="big", signed=False))
        digest.update(encoded)
    return digest.digest()


class _ManualReviewWorkflow:
    """Общий цикл ручной проверки для файлового и прямого режимов."""

    cancel_event: Event
    progress: object
    review_requested: object

    def _init_manual_review(self) -> None:
    ...
    def _review_if_needed(
        ...
        if remembered_approved or remembered_rejected:
            self.progress.emit(
                82,
                f"По сохранённым решениям: подтверждено {remembered_approved:,}, отклонено {remembered_rejected:,}",
            )
        ...
        review_tasks = count_review_record_groups(result for _index, result in review_rows)
        self.progress.emit(
            83,
            f"Требуется ручная проверка: {review_tasks:,} записей ({len(review_rows):,} вариантов)",
        )
        self.review_event.clear()
        self.review_decisions = None
        self.review_requested.emit(review_rows)
        self.review_event.wait()
        if cancelled() or self.review_decisions is None:
...
class _RegistrySourceWorkflow:
    """Общее получение локальных или актуальных реестров НКП."""

    excel_paths: list[str]
    foreign_agents_path: str
    use_nkp_drug_registry: bool
    use_nkp_foreign_agents_registry: bool
    progress: object
...
class ComparisonWorker(QObject, _ManualReviewWorkflow, _RegistrySourceWorkflow):
    ...
    def run(self) -> None:
        try:
            ...
            self._review_if_needed(results, summary, self.cancel_event.is_set)
...
class DirectIrbisComparisonWorker(QObject, _ManualReviewWorkflow, _RegistrySourceWorkflow):
    """Сверяет и изменяет записи на сервере ИРБИС, сохраняя открываемую TXT-копию."""
    ...
    def _save_database_copy(self, records: list[IrbisRecord]) -> Path | None:
        if not records or not self.create_backup:
            return None
        self.backup_dir.mkdir(parents=True, exist_ok=True)
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        target = self.backup_dir / f"irbis_copy_{self.database}_{stamp}.txt"
        write_snapshot_txt(records, target)
        return target
    ...
    def run(self) -> None:
        try:
            ...
            with client as connected:
                ...
                database_copy = self._save_database_copy(irbis_records)
                ...
                scan_marker_neutral_signatures = {
                    record.mfn: _marker_neutral_signature(
                        record,
                        substance_marker=self.substance_marker,
                        foreign_agent_marker_template=self.foreign_agent_marker_template,
                        foreign_organization_marker_template=self.foreign_organization_marker_template,
                ...
                # Перед записью каждый MFN перечитывается, поэтому полные результаты сканирования освобождаем.
                del irbis_records
                ...
                summary.database_copy_file = str(database_copy) if database_copy else ""

                self._review_if_needed(results, summary, self._cancelled)
                ...
                for index, mfn in enumerate(sorted(markers_by_mfn), start=1):
                    ...
                    if scanned_version is not None and live.version != scanned_version:
                        live_marker_neutral_signature = _marker_neutral_signature(
                            live,
                            substance_marker=self.substance_marker,
                            foreign_agent_marker_template=self.foreign_agent_marker_template,
                            foreign_organization_marker_template=self.foreign_organization_marker_template,
                        ...
                        if live_marker_neutral_signature != scan_marker_neutral_signatures.get(int(mfn)):
                            conflicts += 1
                            continue
                        marker_only_version_changes += 1
                ...
                # Контрольное чтение выявляет дубли поля 333, появившиеся на стороне сервера.
                ...
                if marker_only_version_changes:
                    summary.warnings.append(
                        "Повторно обработано записей после изменения только служебных меток: "
                        f"{marker_only_version_changes}."
                    )
                if backup:
