...
# Объединяет создание снимков ИРБИС и перенос изменений из локальных файлов.
class ConnectorWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        logo.setPixmap(QIcon(icon_path("irbis64_control_icon.png")).pixmap(26, 26))
    ...
    def _build_irbis_tab(self) -> QWidget:
        ...
        def toggle_password(visible: bool) -> None:
            ...
            self.show_password.setIcon(QIcon(icon_path("eye-crossed.svg" if visible else "eye.svg")))
        ...
        self.refresh_db_button.setIcon(QIcon(icon_path("refresh.svg")))
        ...
        db_row.addWidget(self.refresh_db_button)
        form.addLayout(db_row, 4, 1)
        form.addWidget(QLabel("Запрос выборки"), 5, 0)
        form.addWidget(self.query_edit, 5, 1)
        ...
        work_layout.addWidget(self._row_with_button(self.snapshot_edit, "Выбрать…", self._pick_snapshot))
        work_layout.addWidget(QLabel("Карта MFN (создаётся автоматически)"))
        work_layout.addWidget(self.manifest_edit)
        work_layout.addWidget(QLabel("Изменённая TXT-копия из ИРБИС64 Контроль"))
        work_layout.addWidget(self._row_with_button(self.modified_edit, "Выбрать…", self._pick_modified))
        ...
        sync = QPushButton("1. Получить базу")
        sync.setObjectName("primary")
        sync.clicked.connect(self.sync_from_irbis)
        apply = QPushButton("2. Отправить TXT в ИРБИС")
        apply.setObjectName("primary")
        apply.clicked.connect(self.apply_to_irbis)
        buttons.addWidget(test)
        buttons.addWidget(sync, 1)
        buttons.addWidget(apply, 1)
        action_layout.addLayout(buttons)
        note = QLabel(
            "Схема: получить базу → проверить TXT в ИРБИС64 Контроль → записать готовую TXT-копию обратно в ИРБИС."
        )
    ...
    def _build_txt_tab(self) -> QWidget:
        ...
        self.local_target_edit = QLineEdit()
        self.local_modified_edit = QLineEdit()
        card_layout.addWidget(QLabel("Исходный TXT-файл"))
        card_layout.addWidget(self._row_with_button(self.local_target_edit, "Выбрать…", self._pick_local_target))
        card_layout.addWidget(QLabel("Новая TXT-копия из ИРБИС64 Контроль"))
        card_layout.addWidget(self._row_with_button(self.local_modified_edit, "Выбрать…", self._pick_local_modified))
        apply = QPushButton("Заменить с резервной копией")
        apply.setObjectName("primary")
        apply.clicked.connect(self.apply_to_txt)
    ...
    def _save_config(self) -> None:
        data = {
            ...
            "modified": self.modified_edit.text().strip(),
        }
        atomic_write_text(
            config_path(),
    ...
    def sync_from_irbis(self) -> None:
        ...
        if not database or not query:
            QMessageBox.warning(self, APP_TITLE, "Обновите список, выберите базу и укажите запрос выборки.")
            return
        self.progress.setValue(0)
        self.log.clear()
        try:
    ...
    def apply_to_irbis(self) -> None:
        ...
        if not modified.is_file() or not manifest.is_file():
            QMessageBox.warning(
                self,
                APP_TITLE,
                "Сначала создайте рабочую базу. Можно отправить исходную, очищенную или изменённую TXT-копию.",
            )
            return
    ...
    def apply_to_txt(self) -> None:
        target = Path(self.local_target_edit.text().strip())
        modified = Path(self.local_modified_edit.text().strip())
        if not target.is_file() or not modified.is_file():
            QMessageBox.warning(self, APP_TITLE, "Выберите существующий исходный TXT и изменённую TXT-копию.")
            return
        try:
