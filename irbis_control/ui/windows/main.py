...
class ApplicationSettingsPage(QWidget):
    ...
    def __init__(
        ...
        # Главное окно уже прокручивает текущую страницу целиком. Второй
        # QScrollArea здесь создавал две соседние вертикальные полосы.
        layout = root
...
class MainWindow(
    ...
    def __init__(self) -> None:
        ...
        # Минимальный размер не даёт контролам схлопнуться, остальное определяет активная страница.
        ...
        self._window_state_autosave_timer = QTimer(self)
        self._window_state_autosave_timer.setSingleShot(True)
        self._window_state_autosave_timer.timeout.connect(self._save_window_state)
    ...
    def _restore_interface_state(self, data: dict[str, object]) -> None:
        ...
        self._fill_path_list(self.database_list, database_paths[:1])
        ...
        for key, handle in (
            ("foreign_agents_list_height", self.foreign_agents_resize_handle),
            ("excel_list_height", self.excel_resize_handle),
            ("match_rules_height", self.match_rules_editor.resize_handle),
        ):
            saved_height = self._safe_int(ui.get(key), 0)
            if saved_height > 0:
                handle.set_target_height(saved_height)
        ...
        self.foreign_agents_upload_toggle.setChecked(bool(ui.get("foreign_agents_upload_collapsed", False)))
        self.excel_upload_toggle.setChecked(bool(ui.get("excel_upload_collapsed", False)))
        self.match_rules_toggle.setChecked(bool(ui.get("match_rules_collapsed", False)))
    ...
    def _save_window_state(self) -> None:
        ...
        payload = {
            ...
            "ui": {
                ...
                "foreign_agents_list_height": self.foreign_agents_list.height(),
                "excel_list_height": self.excel_list.height(),
                "match_rules_height": self.match_rules_editor.rules.height(),
                "foreign_agents_upload_collapsed": self.foreign_agents_upload_toggle.isChecked(),
                "excel_upload_collapsed": self.excel_upload_toggle.isChecked(),
                "match_rules_collapsed": self.match_rules_toggle.isChecked(),
    ...
    def _queue_window_state_autosave(self, *_args) -> None:
        """Сохраняет изменяемые пути и режимы без отдельной кнопки подтверждения."""
        self._window_state_autosave_timer.start(350)
    ...
    def _format_registry_meta(path: Path, cache_path: Path) -> str:
        ...
        try:
            fetched_text = (
                datetime.fromisoformat(fetched.replace("Z", "+00:00")).astimezone().strftime("%d.%m.%Y %H:%M")
            )
        except ValueError:
    ...
    def _nkp_registry_refresh_failed(self, registry: str, error: str) -> None:
        ...
        if self._pending_nkp_download is not None and self._pending_nkp_download[0] == registry:
            self._pending_nkp_download = None
        registry_label = "литература" if registry == "drug" else "издания иностранных агентов"
        runtime_kind = "PyInstaller EXE" if getattr(sys, "frozen", False) else "Python"
        self._append_progress(
            f"Ошибка обновления НКП ({registry_label}). Версия: {APP_VERSION}; "
            f"среда: {runtime_kind}, Python {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}; "
            f"ОС: {platform.platform()}. Причина: {error}"
        )
        self._set_status("Не удалось обновить реестр НКП", "error")
        QMessageBox.warning(
            self,
            APP_TITLE,
            "Не удалось обновить реестр НКП РГБ.\nТехнические подробности записаны в журнал.",
        )
