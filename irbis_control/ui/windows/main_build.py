...
class MainWindowBuildMixin:
    def _build_ui(self) -> None:
        ...
        self.output_edit.textChanged.connect(self._queue_window_state_autosave)
        self.modified_database_edit.textChanged.connect(self._queue_window_state_autosave)
        self.source_mode_combo.currentIndexChanged.connect(self._queue_window_state_autosave)
        self.output_mode_combo.currentIndexChanged.connect(self._queue_window_state_autosave)
        self.workflow_tabs.currentChanged.connect(self._queue_window_state_autosave)
        self.journal_filter_combo.currentIndexChanged.connect(self._queue_window_state_autosave)
        self.journal_search_edit.textChanged.connect(self._queue_window_state_autosave)
        self.scroll_area.verticalScrollBar().valueChanged.connect(self._queue_window_state_autosave)
        self.advanced_scroll.verticalScrollBar().valueChanged.connect(self._queue_window_state_autosave)
        self.run_log.verticalScrollBar().valueChanged.connect(self._queue_window_state_autosave)
        for toggle in (
            self.foreign_agents_upload_toggle,
            self.excel_upload_toggle,
            self.match_rules_toggle,
        ):
            toggle.toggled.connect(self._queue_window_state_autosave)
        for list_widget in (self.database_list, self.foreign_agents_list, self.excel_list):
            model = list_widget.model()
            model.rowsInserted.connect(self._queue_window_state_autosave)
            model.rowsRemoved.connect(self._queue_window_state_autosave)
            model.modelReset.connect(self._queue_window_state_autosave)
            model.dataChanged.connect(self._queue_window_state_autosave)
        for widget in (
    ...
    def _add_collapsible_content_toggle(
        self,
        card: SectionCard,
        content_widget: QWidget,
        label: str,
    ) -> QPushButton:
        """Добавляет в заголовок карточки кнопку сворачивания её крупного содержимого."""
        button = QPushButton("Свернуть")
        button.setObjectName("linkButton")
        button.setCheckable(True)
        link_font = button.font()
        link_font.setUnderline(True)
        button.setFont(link_font)
        button.setToolTip(f"Скрыть раздел «{label}»")
        button.setAccessibleName(f"Свернуть раздел «{label}»")
        button.toggled.connect(
            lambda collapsed: self._set_card_content_collapsed(content_widget, button, label, collapsed)
        )
        card.title_row.setStretch(0, 0)
        card.title_row.insertWidget(1, button, 0, Qt.AlignmentFlag.AlignVCenter)
        card.title_row.addStretch(1)
        return button

    def _set_card_content_collapsed(
        self,
        content_widget: QWidget,
        button: QPushButton,
        label: str,
        ...
        if collapsed:
            ...
            button.setToolTip(f"Показать раздел «{label}»")
            button.setAccessibleName(f"Развернуть раздел «{label}»")
        else:
    ...
    @staticmethod
    def _resizable_list_panel(list_widget: QListWidget, resize_handle: ListResizeHandle) -> QWidget:
        panel = QWidget()
    ...
    def _build_connection_tab(self) -> None:
        ...
        self.irbis_page_size_spin = QSpinBox(self._irbis_state_container)
    ...
    def _build_sources_tab(self) -> None:
        # TXT-источник использует рамку родительской карточки исходных данных.
        self.database_card = QWidget()
        self.database_card.setObjectName("databaseSourcePanel")
        database_body = QVBoxLayout(self.database_card)
        database_body.setContentsMargins(0, 0, 0, 0)
        database_body.setSpacing(4)
        ...
        database_body.addLayout(source_status_row)
        self.database_list = FileDropListWidget(extensions=(".txt",), allow_multiple=False)
        self.database_list.setObjectName("compactList")
        # 27 px соответствует итоговой высоте выпадающих полей темы:
        # min-height 23 px + вертикальные padding и границы.
        self.database_list.setFixedHeight(27)
        self.database_list.setTextElideMode(Qt.TextElideMode.ElideMiddle)
        self.database_list.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        self.database_list.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        self.database_list.setToolTip("Перетащите сюда TXT-файл или нажмите «Добавить TXT»")
        self.database_list.filesDropped.connect(self._drop_database_files)
        self.database_button = QPushButton("Добавить TXT")
        ...
        database_body.addLayout(database_row)
        ...
        self.foreign_agents_resize_handle = ListResizeHandle(
            ...
            minimum_height=28,
            maximum_height=320,
            accessible_name="Изменить высоту списка иностранных агентов",
        )
        ...
        self.foreign_agents_list_panel = self._resizable_list_panel(
            self.foreign_agents_list,
            self.foreign_agents_resize_handle,
        )
        ...
        self.foreign_agents_upload = QWidget()
        fa_actions = QGridLayout(self.foreign_agents_upload)
        fa_actions.setContentsMargins(0, 0, 0, 0)
        fa_actions.setHorizontalSpacing(4)
        fa_actions.setVerticalSpacing(4)
        self.foreign_agents_controls = fa_actions
        self.foreign_agents_card.body.addWidget(self.foreign_agents_upload)
        self.foreign_agents_upload_toggle = self._add_collapsible_content_toggle(
            self.foreign_agents_card,
            self.foreign_agents_upload,
            "Excel",
        )
        ...
        self.excel_resize_handle = ListResizeHandle(
            ...
            minimum_height=28,
            maximum_height=320,
            accessible_name="Изменить высоту списка реестра по наркотическим веществам",
        )
        self.excel_resize_handle.heightChanged.connect(
            lambda height: self._source_list_resized(self.excel_list, height)
        )
        self.excel_list_panel = self._resizable_list_panel(self.excel_list, self.excel_resize_handle)
        ...
        self.excel_upload = QWidget()
        ex_actions = QGridLayout(self.excel_upload)
        ex_actions.setContentsMargins(0, 0, 0, 0)
        ex_actions.setHorizontalSpacing(4)
        ex_actions.setVerticalSpacing(4)
        self.excel_controls = ex_actions
        self.excel_card.body.addWidget(self.excel_upload)
        self.excel_upload_toggle = self._add_collapsible_content_toggle(
            self.excel_card,
            self.excel_upload,
            "Excel",
        )
        ...
        # Layout карточки управляет геометрией редактора и не даёт ему перекрыть заголовок.
        self.match_rules_editor = MatchRulesEditor()
        self.match_rules_editor.resize_handle.heightChanged.connect(self._queue_window_state_autosave)
        match_settings_card.body.addWidget(self.match_rules_editor)
        self.match_rules_toggle = self._add_collapsible_content_toggle(
            match_settings_card,
            self.match_rules_editor,
            "правила",
        )
    ...
    def _build_lists_tab(self) -> None:
        ...
        self.report_sort_combo = AppComboBox()
    ...
    def _build_results_tab(self) -> None:
        ...
        # Эти действия нужны логике состояния, но не выводятся в блоке запуска.
        ...
        self.open_modified_database_button = QPushButton("Открыть копию базы данных")
        ...
        self.journal_filter_combo = AppComboBox()
