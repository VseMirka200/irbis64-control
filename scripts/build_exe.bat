...
rem Build the standalone database connector placed next to the main EXE.
"%VENV_PY%" -m PyInstaller --noconfirm --clean --onefile --windowed ^
...
echo   Main application: dist\IRBIS64Control ^(Russian filename^).exe
echo   Database connector: dist\IRBIS64ControlDB.exe
echo.
if /I not "%~1"=="--no-pause" pause
exit /b 0
